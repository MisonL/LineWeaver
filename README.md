...
LineWeaver对Markdown文档的处理采用"先保护，后处理，再恢复"的策略，确保去除换行符后仍能保留Markdown的结构。

### Markdown处理核心机制

1. **保护阶段**：使用占位符保护特殊结构
   - 代码块：`__CODE_BLOCK_0__`
   - 表格行：`__TABLE_ROW_0__`
   - 标题、引用等
...
| Markdown元素 | 处理方式 | 输出效果 |
|------------|---------|---------|
| 标题(#) | 添加[PARA]前缀 | [PARA]# 标题 |
| 段落 | 段落间添加[PARA] | 段落1[PARA]段落2 |
| 无序列表 | 列表项添加[LIST] | [LIST]- 项目1 |
| 有序列表 | 列表项添加[LIST] | [LIST]1. 项目1 |
| 代码块 | 转换为[CODE]...[/CODE] | [CODE]```代码```[/CODE] |
| 行内代码 | 保留原样 | `代码` |
| 表格 | 保留换行符(\n) | \|表头\|\n\|--\| |
| 引用(>) | 添加[LIST] | [LIST]> 引用内容 |
| 水平线 | 添加[PARA]前后缀 | [PARA]---[PARA] |
| 链接和图片 | 保留结构，合并内部空格 | [文本](链接) |

### Markdown处理优势

1. **结构完整性**：即使转换为单行文本，也能保持Markdown的结构信息
2. **AI友好**：特殊标记帮助AI工具理解文档结构
3. **可读性**：转换后的文本仍保持一定的可读性
4. **可逆性**：通过标记可以推断原始文档结构
5. **智能处理**：针对不同Markdown元素采用不同处理策略

### 使用建议

- **完整文档**：使用智能模式处理整个Markdown文档
- **代码分享**：包含代码块的文本推荐使用智能模式
- **表格数据**：表格数据会自动保留格式
- **大型文档**：分段处理大型文档，每次粘贴一个逻辑部分
...
#### 🛠️ 功能改进
- **默认选择智能模式**：现在默认选择智能模式，提供更好的文本结构保留
- **Markdown表格支持**：优化了对Markdown表格的处理
- **代码块处理增强**：改进了代码块内容的保护和恢复机制
- **错误处理优化**：增强了错误处理机制，提供更精确的错误信息
- **用户界面优化**：简化了错误提示，提高用户体验

#### 🐞 问题修复
- 修复了`tableRows`未定义导致的转换错误
- 修复了Safari浏览器下粘贴功能兼容性问题
- 移除了错误检测和拦截机制，允许处理任何类型的文本内容
- 优化了变量初始化顺序，提高代码健壮性
- 改进了错误反馈机制，更友好地展示错误信息

#### 📚 文档更新
- 更新了README文档，添加了详细的Markdown处理说明
- 增加了更多使用示例和最佳实践
- 优化了部署指南和浏览器兼容性说明
